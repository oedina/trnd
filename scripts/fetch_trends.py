...
def http_get(url, headers=None, timeout=20):
    """Simple HTTP GET, returns response text or None."""
    try:
...
def fetch_tiktok():
    """
    Scrape TikTok Creative Center trending keywords.
    URL: https://ads.tiktok.com/business/creativecenter/trend-keywords/pc/en
    Falls back to hashtag view count estimation via public search.
    """
    ...
    for style in STYLES:
        ...
        try:
            # TikTok Creative Center keyword search
            ...
            if html:
                try:
                    ...
                    # Extract popularity score from response
            ...
            # Also try public hashtag page for view count signal
            tag_url = f"https://www.tiktok.com/tag/{tag}"
            tag_html = http_get(tag_url)
            if tag_html:
                # Look for view count in page JSON
                match = re.search(r'"videoCount"[:\s]+(\d+)', tag_html)
                if match:
                    video_count = int(match.group(1))
                    # Normalize video count to 0-100 range (10M videos = ~100)
...
def fetch_youtube():
    """
    Use YouTube Data API v3 to get video count + view signals.
    Free quota: 10,000 units/day. Each search = 100 units.
    20 styles × 100 = 2,000 units/week. Well within free limit.
    """
    ...
    for style in STYLES:
        try:
            ...
            # Search for videos published in last 7 days
            url = (
            ...
            if items:
                ...
                if video_ids:
                    ...
                    if stats_resp:
                        ...
                        # Score: video count (0-10 → 0-50) + view signal (0-50)
...
def fetch_pinterest():
    """
    Pinterest trends via their public trends endpoint.
    No API key needed for basic trend data.
    """
...
def fetch_photos():
    ...
    Free tier: 50 requests/hour. We use 20/week. Well within limit.
    Photos saved to images/ folder in repo root.
    """
    print("\n[5/5] Unsplash photos...")
    ...
    for style in STYLES:
        # Skip if photo already exists (don't re-fetch every week)
        filename = style["name"].lower().replace(" ", "-").replace("'", "") + ".jpg"
        filepath = f"../images/{filename}"
        if os.path.exists(filepath):
            print(f"  ✓ {style['name']}: already exists")
            photos[style["name"]] = f"images/{filename}"
            continue
        ...
        try:
            ...
            if resp:
                ...
                if results:
                    # Get a medium-sized image URL
                    img_url = results[0]["urls"]["regular"]
                    photo_credit = results[0]["user"]["name"]

                    # Download and save the image
                    img_data = http_get(img_url, headers={})
                    if img_data:
                        with open(filepath, "wb") as f:
                            f.write(img_data.encode("latin-1") if isinstance(img_data, str) else img_data)
                        photos[style["name"]] = f"images/{filename}"
                        print(f"  ✓ {style['name']}: saved (photo by {photo_credit})")
                    else:
                        photos[style["name"]] = ""
                else:
                    print(f"  - {style['name']}: no results")
                    photos[style["name"]] = ""
            else:
                photos[style["name"]] = ""
...
def fetch_gemini_insights(scored_styles):
    """Generate one-line insight per top-10 style using Gemini Flash."""
...
def combine_scores(google, tiktok, youtube, pinterest, photos, prev_data):
    """Merge all signals into final ranked list."""
    ...
    for style in STYLES:
        ...
        final = round(g * WEIGHTS["google"] + t * WEIGHTS["tiktok"] +
                      y * WEIGHTS["youtube"] + p * WEIGHTS["pinterest"])
    ...
    for i, s in enumerate(styled):
        ...
        s["momentum_pct"] = round(
            (s["final_score"] - (prev or s["final_score"])) / (prev or s["final_score"] or 1) * 100, 1
        ) if prev else 0.0
...
def save_data(styled, insights):
    ...
    # 1. Save current week as data.json (homepage reads this)
    with open("../data.json", "w") as f:
        json.dump(output, f, indent=2)
    print(f"\n  ✓ data.json saved")

    # 2. Save to rankings/YYYY-MM-DD.json (history archive)
    os.makedirs("../rankings", exist_ok=True)
    week_file = f"../rankings/{now.strftime('%Y-%m-%d')}.json"
    with open(week_file, "w") as f:
        json.dump(output, f, indent=2)
    print(f"  ✓ rankings/{now.strftime('%Y-%m-%d')}.json saved")

    # 3. Update rankings/index.json (list of all past weeks)
    index_file = "../rankings/index.json"
    try:
...
def main():
    ...
    print(f"Sources: Google({WEIGHTS['google']*100:.0f}%) + TikTok({WEIGHTS['tiktok']*100:.0f}%) + YouTube({WEIGHTS['youtube']*100:.0f}%) + Pinterest({WEIGHTS['pinterest']*100:.0f}%)")
    print("=" * 55)
